class WCIntroductionResume extends HTMLElement {
...
const attachListeners = (o) => {
    ...
    o.shadow.addEventListener('click', (evt) => {
        if (evt.target.classList.contains('switch-on')) {
            let spans = Array.from(o.shadow.querySelectorAll('ul > li span'))
            spans.forEach(span => span.style.display = 'block')
        }
        if (evt.target.classList.contains('switch-off')) {
            let spans = Array.from(o.shadow.querySelectorAll('ul > li span'))
...
customElements.define('wc-introduction-resume', WCIntroductionResume);
