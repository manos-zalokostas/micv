...
$(document).ready(function() {		
		...
		$("#skill_fields b").each(function(){
		$(this).click(function(){
			var pause_btn = $('#skills_preview > div > em');
			if($(pause_btn).hasClass('paused')) {handle_pause_action()}
			animate_skills('off');
		...
		function handle_pause_action(){
			...
			if(pause_btn.attr('class')){
					...
					animation_running =true ; animate_skills(gmode);				
			}
			else{
					$(pause_btn).addClass('paused');
					animation_running = false; animate_skills('off')
			}
		}		

		// LISTENER FOR THE SKILLS - BANNER 
		$('#skills_preview').mouseenter(function(){
			var pause_btn = $('#skills_preview > div > em');
			if($(pause_btn).hasClass('paused')) return;
			$(pause_btn).css('display','block');
			})

			$('#skills_preview').mouseleave(function(){
			var pause_btn = $('#skills_preview > div > em');
			if($(pause_btn).hasClass('paused')) return;
		...
		$('#cv_comments').click(
			function(){
				...
				$('#cv_description i').css({'display':'block'}).animate({'opacity':'1'});
					...
					$('#cv_description i').animate({'opacity':'0'}, function(){$(this).css({'display':'none'})});
		...
		$("#timeline_btns a").each(function(){
			$(this).click(function(){
						var domain = this.id;
						
						if(domain == 'mlt') color ='gainsboro';
						else if(domain =='std') color = 'teal';
						...
						if(document.querySelector('#timeline_btns a[style]')){document.querySelector('#timeline_btns a[style]').removeAttribute('style')}						
		...
		//MAIN BUTTONS 'CLICK'		
		$("#menu_tabs li a").click(function (event){
						
						if(document.querySelector('#context').style.left != 0) {$('#context').animate({'left':0})}
						nav_bar_designer(event.target.innerHTML, 'navigation') ;
		});
		
		// LIST BUTTONS 'CLICK'
		$('#list').delegate('.sublist>li', 'click', function (event) {
		...
		// LIST BUTTONS 'MOUSEENTER'
 		$('#list').delegate('ul.list>li, ul.sublist>li', 'mouseenter', (function (event) {		
		...
		// LIST BUTTON 'MOUSELEAVE'
		 $('#list').delegate('ul.list>li', 'mouseleave', (function (event) {
		...
		// EXTRA BUTTONS 'CLICK'
		$('#iextra > div').click(function(event){
		
						preview_extras(this);												 
		})
		
		// media / files / IMAGES  ELEMENTS 'CLICK'
		$('#imedia, #ifiles, #iscreenshots').delegate( 'a', 'click', function(event){
		
						var item_src = $(this).attr('href'); 
						var app ="";
						if($(this).parent().parent().attr('id').search('media') > -1) app = 'avi';
						else if($(this).parent().parent().attr('id').search('files') > -1) app = 'pdf';
						else app = 'jpg';
		...
		$('#description').delegate('a.keys', 'click', function (event){
		...
		$('#description').delegate('a.cat_key', 'click', function (event){
		...
		$('#list').delegate('#temp_list li', 'click', function(event){
...
function navigate_resume_page(domain){
				...
				if(document.querySelector('#introduction_menu a.selected')){document.querySelector('#introduction_menu a.selected').removeAttribute('class')}						
				...
				$(domain).siblings().each(function(){
					if(this.id.match('cv_field')){
					$(this).animate({'opacity':'0', 'left':'-100%'}, 'swing', function(){	
					})
					}
				})
				
				$(domain).animate({'opacity':'1', 'left':'0'});
				$(domain).find('.main_txt > p').animate({'left':0}, function(){$(domain).find('.main_txt > h2').animate({'opacity':'1'})});
				$(domain).find('.aux_txt > p').animate({'right':0},function(){$(domain).find('.aux_txt > h3').animate({'opacity':'1'})});
...
function animate_page (current_page){
				...
				if($('#site_menu a.selected').attr('title') == current_page ){ return;}
					...
					$('#site_menu a[title="'+current_page+'"]').addClass('selected');
						...
						case 'about' :	
									$('#wrapper').animate({'top':'0%'},'slow', 'swing', function(){$('#site_menu').animate({'top': 0 } )} );
									$('#author_presentation > img').animate({'left':0}, 'slow');
									if( ! animation_running && !animation_paused){animate_skills(gmode);}
						break;									
						case 'projects' :	
									$('#wrapper').animate({'top':'-100%'},'slow', 'swing');
									$('#author_presentation > img').animate({'left':'-100%'}, 'slow', function(){$('#site_menu').animate({'top':0})});
									animate_skills('off');
						break;									
						case 'certificates' :	
									$('#wrapper').animate({'top':'-200%'},'slow', 'swing',function(){$('#site_menu').animate({'top':0})});
...
function ajax_retrieve_skill_data(){
				...
				$.ajax({	   		  										
				url: 'manos_skills.xml',
				dataType: 'xml',
				success: function (data){ 
					var psize=0, tsize=0, rnum =0;
					...
					$(data).find('item').each(
							function(){
							proj.name = $(this).children('title').text();
							proj.cat = $(this).children('category').text();
							proj.desc =".." + $(this).children('description').text().substring($(this).children('description').text().indexOf('<span>')+6, $(this).children('description').text().indexOf('</span>')) +"..";
							proj.img = $(this).children('screenshots').children('shot:first-child').text();
							projs.push(proj);
							proj ={};
					});
					
					projects = projs.slice(); 
						temp_projects = projs.slice();
					...
					tech_tools = tools.slice();
						temp_tools = tools.slice();
				},
				complete: 
					function(){
...
function proccess_ajax_data(field){
				...
				if(field == 'global') {prun=true, crun=true, trun=true}
				else if (field == 'project') {prun = true}
				else if (field == 'reference') {crun = true}
				else if (field == 'tool') {trun = true}
				else (alert('Specify FIELD to proccess'))

				// PROCCESS THE "PROJECTS" FIELD
				if(prun){				
					procstr = "p";
							psize = temp_projects.length-1;
							if(psize <= 0) {
								temp_projects = projects.slice();
								psize = projects.length -1;
							}
							rnum = Math.floor((Math.random()*psize)+1);			
							var currp =  $(temp_projects[rnum])[0];
				...
				if(crun){				
					...
					procstr += "c";
								csize = temp_references.length-1;
								if(csize <=0) {
								temp_references = references.slice();
								csize = references.length -1;
								}				
								rnum = Math.floor((Math.random()*csize)+1);			
								// DESIGNATE THE 'PROJECT' OBJECT FOR 1ST-PAGE SHOWCASE
								currc = temp_references[rnum];
								cname= currc[1];
								cdata = currc[0];
								
								// THIS WILL BE USED TO REMOVE DUBLICATE-ENTRIES FROM THE ARRAY BUILD
								ckey = cname.substring(cname.indexOf('">')+2, cname.indexOf(','));
							
								c.name = cname.substring(0, cname.indexOf('</h4>')+5);
								c.label = cname.substring(cname.indexOf('<p>'), cname.indexOf('</p>')+4);
								c.img = (cname.match('linkedin')>-1) ? 'images/tech_logos/linkedin_user.jpg' : 'images/tech_logos/facebook_user.jpg';
								c.data = cdata;
								c.item = currc[2];
								// THROUGH (POP) THE ITEM FROM THE ARRAY, SO THAT IT WILL NOT BE RETRIEVED AGAIN LATER
								temp_references.splice(rnum, 1);
				}
				...
				if(trun){	
					procstr += "t";
								...
								isize = $('#skills_preview #tool h4').length;
			...
			function convert_data_to_html (project, comment, tools, field, proccess_string) {
				...
				if(pct.indexOf('p')>-1){
						...
						pct = pct.replace('p', '');	
						if(pct=='' && !animation_running){gmode='project'; animate_skills(gmode); return;}
				}
				...
				if(pct.indexOf('c')>-1){
						...
						pct = pct.replace('c', '');	
						if(pct=='' && !animation_running){gmode='reference'; animate_skills(gmode); return;}
				}
	...
	function place_skill_images( divs, skills, counter, field, pctl){
					//console.log(divs, skills, counter);
					var node ={};
					var name = '';
...
function animate_skills(mode){
				...
				var animate_next = true;
					...
					max = $('#skills_preview> ul> li').length;
					 ...
					 if(mode=='reference'){sindex +=1}
					 else if(mode=='tool'){sindex +=2}
					 else {sindex=sindex}
					max=1;
					}
				}
				...
				(function(){
					...
					animation_running = true;
									if(mode=='off') {
											...
											if(counter >=max) {	proccess_ajax_data(mode); index=sindex; counter =0/* mode='off';*/  /*animate_next=false;*/}
											...
											$(anime_child).attr('class','slide_animated').animate({'left':0}, function(){
		...
		function configure_banner_display(filter){
				if(gmode == 'global' || gmode != filter){
					gmode = filter;
					if($('.filteron')){$('.filteron').removeAttr('class')}
...
// HOVER THE MAIN BUTTONS (3) OF 'PROJECTS' PAGE (p2)	
function hover_in_menu_buttons (event){
									//console.log(this);
									$(this).css({'background':'url("./images/_buttons/'+
									event.target.innerHTML.toLowerCase()+'_btn.png") no-repeat 5% 0'
									})
									.animate({'padding-top':'20px'}, 'medium')
}

// HOVER OUT THE MAIN BUTTONS (3) OF 'PROJECTS' PAGE (p2)	
function hover_out_menu_buttons (event){//alert('left')							
			
									$(this).animate({'padding-top':'0'}, 'fast', function(){$(this).stop()})		
...
function preview_extras(element) {
									...
									$(elem).attr('class','extra_preview').animate({'height': '80%'},'medium','linear',	function(){
...
function nav_bar_designer(item_requested, function_caller) {
					...
					clean_page_data();
						...
						if($('#media_control').attr('id')) { slide_images('x', null) };
									...
									success: function (data){ 
												...
												function(){
													if($(this)[0].textContent== item){
														console.log($(this)/*, item +  "  " + $(this).parent().siblings('title') */);													
												...
												$(childs).each( function(){temp_array.push($(this).text());})
									},
									complete: function(){
												...
												$('#list ul li').each(function(){$(this).animate({'left':0},'slow')}); 						
									},
									error: function(){
...
function clean_page_data(){
								...
								$(id_elems).each(function(){$(this).css({'display':'none'})});					
								$(elem_spans).each(function(){$(this).html('')});	
...
function build_selected_item_content(current_list_item, curr_list) {
																	...
																	if(this.childNodes[1] && (this.childNodes[1].nodeType =='1')){ 
																			...
																			var mdata = "";
																				...
																				switch(tag){
																					...
																					case 'screenshots':
																								$(data).each(function() {
																								mdata +='<a href="'+$(this).text()+'"><img  src="' + $(this).text() + '" title="'+$(this).text().substring($(this).text().lastIndexOf('/')+1, $(this).text().lastIndexOf('.')).replace(/_/g, " ")+'"  /></a>'; 																	
																					...
																					data= $(this).text();
																					if(data !='' && data !='-'){
																							if(tag =='tutor') {$('#icomplements').css({'display':'block'})}
																							$('#description #i' + tag).css({'display':'block'});
																							$('#description #i' + tag + ' span').html(data);
																							if(tag=='link'){$('#description #i' + tag + ' span').html('<a href="'+data+'">VISIT ONLINE</a>');}
																					}
																	}
													});
												//END OF 'AJAX' FUNCTION
												...
												if(list){
													...
													$('#context').animate({'left':'-100%'}, function(){preview_extras(field)});															
													return;
												}
...
function open_field_on_anime_end(list){
			...
			var field = {};
				...
				case 'web': field = $('#ilink'); break;
				case 'studies': field = $('#icomplements'); break;
				case 'work': field = $('#imedia'); break;
				default: field = $('#ifiles'); break;
				}
				return field;
}			
...
function content_handler (item_src, app) { 
											...
											var item;
								 			...
								 			if(src == 'images/cv_document/micv.pdf'){$('#iextra > div').css('display','none')}
									
								 			//ANIMATES THE OPACTITY OF THE 'TEXT and IMAGES' BACKGROUND RESOURCES 
											//BOFORE THE SLIDESHOW AREA IS EXPANDED
											$('#ibody > div:first-child').children().animate({opacity: 0}, 'normal', function() { 
										...
										if( ! ($('#media_control').is(':visible'))) {
												...
												$('#media_control').animate({'left': '51%'}, 'linear', function(){			
											...
											if($(image_parent).siblings().length > 0) {
								 			...
								 			$('#media_control span').click(function (event){
...
function slide_images(direction, button_clicked) {
										...
										var btnprev = $('#cpanel span[title="previous"]');
							 			...
							 			if(handler == "next") {
							 				...
							 				if($(selected_item).next().attr('href')){ // alert(' SIBILI');
											...
											$('#preview').children(':first').animate({'opacity':0, 'right': '100%'}, function(){$('#preview').children(':first').attr('src', src).css({ 'right':'-100%'})}).animate({'opacity':1, 'right':0});
											}
											else {
											...
											$('#media_control').animate({height: 0},'fast', function(){
