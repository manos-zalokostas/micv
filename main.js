function initializeListeners() {
    ...
    rootElement.addEventListener('click', (event) => {
        ...
        if (target.matches("#skill_fields b")) {
            ...
            animateSkills('off');
            configureBannerDisplay(target.getAttribute('title'));
        }
        ...
        if (target.closest('#skills_preview ul>li>div')) {
            handleBannerInput(target.parentNode.id);
        }
        ...
        if (target.matches('#cv_comments')) {
            ...
            if (cvDescriptionI.style.display === 'none' || !cvDescriptionI.hasAttribute('style')) {
                ...
                animateOpacity(cvDescriptionI, 1);
            } else {
                animateOpacity(cvDescriptionI, 0, () => {
...
// LISTENERS FOR THE TIMELINE DATES (2001, 2005 etc) TO DELIVER (ONCLICKED) THEIR CONTENT ONSCREEN
        ...
        if (target.tagName === 'LI' && target.parentNode.classList.contains('sublist')) {
            ...
            if (target.tagName === 'A' && (target.parentElement.id === 'imedia' || target.parentElement.id === 'ifiles' || target.parentElement.id === 'iscreenshots')) {
                var item_src = target.getAttribute('href');
                var app = "";
...
function handleInputListChange() {
    if (this.getAttribute('list') === 'project_list') {
        handleBannerInput('project', this.value.split(' ').shift());
    }
    if (this.getAttribute('list') === 'skill_list') {
        handleBannerInput('tool', this.value.split(' ').shift());
    }
    document.querySelector("input[list]").value = '';
}

// Function to be implemented based on your specific requirements
function animatePage(currentPage) {
    // Implement the animatePage function
}

// Function to be implemented based on your specific requirements
function handleBannerInput(type, value) {
    // Implement the handleBannerInput function
}


// Function to be implemented based on your specific requirements
function handlePauseAction() {
    // Implement the handlePauseAction function
}

// Function to be implemented based on your specific requirements
function animateSkills(action) {
    // Implement the animateSkills function
}

// Function to be implemented based on your specific requirements
function configureBannerDisplay(title) {
    // Implement the configureBannerDisplay function
}

function navigateResumePage(domain) {
    // Implement the navigateResumePage function
}

// Function to be implemented based on your specific requirements
function animateOpacity(element, opacity, callback) {
    // Implement the animateOpacity function (e.g., using CSS transitions)
}